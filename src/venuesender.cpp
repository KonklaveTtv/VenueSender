...
void promptUserForGenreFilter(const std::vector<std::string>& uniqueGenres, std::string& selectedGenre) {
    ...
    std::cout << "Enter the genre to filter by: ";
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    std::getline(std::cin, selectedGenre);

    // Call clearInputBuffer() after user input
    clearInputBuffer();
}
...
void promptUserForStateFilter(const std::vector<std::string>& uniqueStates, std::string& selectedState) {
    ...
    std::cout << "Enter the state to filter by: ";
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
...
void promptUserForCityFilter(const std::vector<std::string>& uniqueCities, std::string& selectedCity) {
    ...
    std::cout << "Enter the city to filter by: ";
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    std::getline(std::cin, selectedCity);
...
void promptUserForCapacityFilter(const std::vector<int>& uniqueCapacities, int& selectedCapacity) {
    // Display available capacities to the user
    std::cout << "Available Capacities: ";
    for (int capacity : uniqueCapacities) {
    ...
    // Prompt the user to select a capacity
    while (true) {
        std::cout << "Enter the capacity to filter by (0 to cancel): ";
        int capacityChoice;
        if (std::cin >> capacityChoice) {
            if (capacityChoice == 0) {
                std::cout << "Capacity filter canceled." << std::endl;
                selectedCapacity = INVALID_CAPACITY;
                return;
            }
            // Check if the selected capacity is valid
            auto it = std::find(uniqueCapacities.begin(), uniqueCapacities.end(), capacityChoice);
            if (it != uniqueCapacities.end() && capacityChoice != INVALID_CAPACITY) {
                selectedCapacity = capacityChoice;
                return;
            } else {
                std::cout << "Invalid capacity. Please select from the available options." << std::endl;
                std::cin.clear();
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            }
        } else {
...
void promptUserForFilters(const std::vector<std::string>& uniqueGenres,
    ...
    while (true) {
        ...
        std::getline(std::cin, selectedCriteria.genre);

        if (std::find(uniqueGenres.begin(), uniqueGenres.end(), selectedCriteria.genre) == uniqueGenres.end()) {
            std::cout << "Invalid Genre. Please select from the available options." << std::endl;
            continue;
        }
        ...
        std::cout << "Select a State: ";
        std::getline(std::cin, selectedCriteria.state);

        if (std::find(uniqueStates.begin(), uniqueStates.end(), selectedCriteria.state) == uniqueStates.end()) {
            std::cout << "Invalid State. Please select from the available options." << std::endl;
            continue;
        }
        ...
        std::getline(std::cin, selectedCriteria.city);

        if (std::find(uniqueCities.begin(), uniqueCities.end(), selectedCriteria.city) == uniqueCities.end()) {
            std::cout << "Invalid City. Please select from the available options." << std::endl;
            continue;
        }

        std::cout << "Available Capacities:" << std::endl;
        for (const std::string& capacity : uniqueCapacities) {
            std::cout << "- " << capacity << std::endl;
        }
        
        int selectedCapacity;
        while (true) {
            std::cout << "Select a Capacity: ";
            if (!(std::cin >> selectedCapacity)) {
                std::cout << "Invalid input. Please enter a valid capacity." << std::endl;
                clearInputBuffer();
                continue;
            }
            
            if (selectedCapacity <= 0) {
                std::cout << "Invalid capacity. Please enter a positive value." << std::endl;
                clearInputBuffer();
                continue;
            }
        ...
        selectedCriteria.capacity = selectedCapacity;
        break; // Exit the loop when valid input is provided for all criteria
    }
}
