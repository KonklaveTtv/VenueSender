...
bool EncryptionManager::encryptPassword(const string& decryptedPassword, string& encryptedPassword) {
    ...
    // Encrypt the password using the encryption key and nonce
    ...
    string nonceStr(reinterpret_cast<const char*>(encryptionNonce.data()), encryptionNonce.size());
...
string EncryptionManager::decryptPassword(const string& encryptedPassword) {
    ...
    // Decrypt the ciphertext using the encryption key and extracted nonce
